...
export async function getTod(id){
...
export async function getSharedTodoByID(id){
 ...
 export async function createTod(user_id, title){
 ...
 export async function toggleComplete(id, value) {
    ...
    const [result] = await pool.query(
        `UPDATE todos SET completed = ${newValue}
        WHERE id = ?;
        `,
        [id]
    );
 ...
 export async function shareTod(todo_id, user_id, shared_with_id){
    const [result] = await pool.query(
        `
